...
import { submitRequest, getRequestById, getAllRequests, getRequestsByUserId, getAllPendingRequestCounts, reviewRequest } from "../controllers/requestController";
...
router.get("/", authorizeRoles(Role.admin, Role.tl, Role.nurse, Role.hr), getAllRequests);

// GET /api/requests/pending-counts
// Grouped pending request counts by type
router.get("/pending-counts", authorizeRoles(Role.admin, Role.tl, Role.nurse, Role.hr), getAllPendingRequestCounts);

// Get all requests
// GET /api/requests
// Returns paginated list with filters by role
router.get("/user/:userId", authorizeRoles(Role.admin, Role.tl, Role.hr, Role.nurse), getRequestsByUserId);
