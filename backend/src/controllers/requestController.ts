...
export const submitRequest = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Notify TL, Nurse, Admin RH
    const notifyRoles = [Role.tl, Role.nurse, Role.admin]; // import Role enum
...
export const reviewRequest = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(200).json({ message: "Demande revue avec succès." });
  } catch (err) {
