...
export const patchMyProfile = async (req: Request, res: Response) => {
  try {
    ...
    const forbiddenFields = ["password", "role", "department", "email", "status"];
    ...
    if (data.firstName || data.lastName) {
      ...
      data.fullName = `${data.firstName || existingUser?.firstName || ""} ${data.lastName || existingUser?.lastName || ""}`.trim();
    }
...
export const getMyRequestById = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!request || request.userId !== userId) {
      return res.status(403).json({ message: "Requête non autorisée ou introuvable." });
    }
...
export const patchMyRequest = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (existing.status !== "pending") {
      return res.status(400).json({ message: "Impossible de modifier une requête non en attente." });
    }
...
export const deleteMyRequest = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (existing.status !== "pending") {
      return res.status(400).json({ message: "Impossible de supprimer une requête non en attente." });
    }
